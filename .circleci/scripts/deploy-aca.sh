...
print_changed_resources() {
  local plan_json="$1"

  printf 'Terraform changed resources:\n' >&2
  jq -r '
    def path_text:
      reduce .[] as $part ("";
...
inspect_plan_json() {
  ...
  if ! print_changed_resources "$plan_json"; then
    printf 'Unable to summarize Terraform resource changes safely in phase %s; operation refused.\n' \
      "$phase_name" >&2
    return 1
  fi
