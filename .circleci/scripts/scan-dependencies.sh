...
readonly MAVEN_REPOSITORY="${HOME}/.m2/repository"

ensure_jq
mkdir -p reports .trivy-cache

ensure_resolved_maven_cache() {
  if [[ ! -d "$MAVEN_REPOSITORY" ]] || \
      [[ -z "$(find "$MAVEN_REPOSITORY" -type f -name '*.pom' -print -quit)" ]]; then
    echo "Maven cache is unresolved; the SCA scan is incomplete." >&2
    return 1
  fi
}
...
validate_dependency_report() {
  ...
  # On the first workflow for a new cache key, backend-build-test should make
  # the Maven cache warm before this job starts. Fail explicitly if that
  # guarantee is ever broken instead of accepting a direct-only SCA report.
  jq -e '
    any(
      .Results[]?;
      .Type == "pom"
  ...
  ' "$TRIVY_REPORT" >/dev/null || {
    echo "Maven cache is unresolved; the SCA scan is incomplete (no transitive backend dependencies found)." >&2
    return 1
  }
}
